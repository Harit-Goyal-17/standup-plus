...
export default function MyListPage() {
  ...
  return (
    <div style={{ paddingTop: '80px', paddingBottom: '40px', minHeight: '100vh' }}>
      <h1 style={{ fontFamily: 'var(--font-display)', fontSize: '2.5rem', marginBottom: 32, paddingLeft: '4%' }}>My List</h1>
      ...
      {favorites.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '60px 20px', background: 'var(--bg-card)', borderRadius: 12, margin: '0 4%' }}>
          ...
          <Link to="/shows" className="btn-primary">Browse Specials</Link>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '40px' }}>
          <VideoCarousel title="Saved to My List" videos={favorites} onVideoClick={handleVideoClick} />
        </div>
      )}
    </div>
  );
