...
async function inspect() {
  ...
  const comedians = query(`SELECT comedian_id, name, profile_image_url FROM comedians ORDER BY name`);
  ...
  for (const name of targetNames) {
    const found = comedians.find(c => c.name.toLowerCase().includes(name.toLowerCase().split(' ')[0]));
    if (found) {
      ...
      vids.slice(0, 10).forEach(v => console.log(`  - [${v.video_id}] ${v.title} (${Math.round(v.duration_seconds/60)}m, ${v.view_count} views, thumb: ${v.thumbnail_url})`));
    } else {
